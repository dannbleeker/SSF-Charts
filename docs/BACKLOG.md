...
> when its evidence lives in another document.
    ...
    22  the store screenshots need a person at a real PowerPoint window
       — `docs/STORE-LISTING.md` has carried this as blocked since it was
       written and it is not a task an agent can take. The slide canvas does
       not composite in a headless browser: every capture shows the ribbon and
       the pane correctly and a uniform #F5F5F5 where the slide should be. The
       pane alone IS capturable and is captured every round; a shot showing an
       inserted chart is not. 1-5 at 1366x768.
