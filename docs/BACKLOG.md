...
> **AND IT WENT STALE A FOURTH WAY, 2026-09-13 — this time not by miscounting
> but by not looking.** The list was headed "as of 2026-09-06" and still said
> ONE. In the week between, the product was taken to the edge of publication:
> the manifest floor moved 1.4 → 1.8 → 1.10, the first non-web host reading was
> taken, Mac was found to be unmeasurable by its owner and measured at the
> engine instead, and a store listing was corrected twice. Not one of those
> landed here, because they were tracked in `PUBLISHING.md` and
> `STORE-LISTING.md` where the work happened. So the register answered "one
> thing is open" to a question whose real answer was six, and it took the owner
> asking "what is open" for anyone to read it. **The trap this file keeps
...
> the next reader will meet the same formatting.
    ...
    5  filing this project's host measurements to the office-js tracker
       ...
       DRAFT C NEEDS NOTHING BUT HIS IDENTITY (checked 2026-09-13): office-js
       #6237 is still OPEN, labelled "Type: product bug / Area: PowerPoint",
       with Microsoft activity on 2026-08-31, and the comment is written. It
    ...
    21  the Mac host surface has never been measured, and cannot be here
       ...
       + iPad as one unit, and there is no platform scoping — office-js#6658),
       and certification policy 1120.3 names Mac explicitly. So it is a
       decision — submit with it unmeasured, or buy a reading — and therefore
       the owner's.
       WHAT NARROWS IT, both done 2026-09-13. The floor at 1.10 means every
       certified host now runs the SAME code path; at 1.8 the picture fallback
       was Mac-only code on the unmeasured platform. And
    ...
    22  the store screenshots, and the choice of how to make them
       ...
       FALSE, corrected the same evening. True: the slide canvas does not
       composite in a headless browser, so every browser capture is blank where
       the slide should be. False: "a shot showing an inserted chart is not
       capturable". `slideImageBase64` renders a slide through Office.js rather
       than the screen — `Slide.getImageAsBase64`, PowerPointApi 1.8, at a
       caller-chosen width — and every round already produces them; the archive
       strips them only because they are 48% of the bytes.
       SO IT IS A CHOICE, NOT A BLOCK. Pane and slide are each capturable; what
       is unavailable headlessly is one AUTHENTIC frame holding both. Three
       routes, argued in STORE-LISTING.md: a real screenshot on AITEST (needs
       the owner to connect a viewer once), separate images, or compositing the
...
| 10 | ~~**What a host below PowerPointApi 1.10 loses — 18 of 123 charts, 9 of them wholly**~~ | §1, *Below 1.10* | **DECIDED AND SHIPPED 2026-08-31 — the owner chose WARN AND PICTURE.** A chart carrying marks this host cannot draw now inserts as a PICTURE, complete, with a message naming what would have been missing ('This PowerPoint cannot draw 4 pie slices...'). Where no picture is possible it still draws and still says what is absent. And Explode refuses on the same test: exploding such a chart is the door back to the version that cannot be drawn, which is the mistake the density guard's own comment records the product making once before. Evidence below is what the decision was made on. The renderer cannot rotate a shape below 1.10, so `addWedgeFan` and the arrowhead case both trace and return nothing. **Measured, not estimated:** 18 of the 123 shipped charts lose ink — pie 4/4, doughnut 2/2, sunburst 2/2, radar 1/5 (those **9 keep only their labels**, the wedge IS the chart), then gantt 4/7, stacked 3/14, waterfall 1/6, scatter 1/8, which lose annotation arrows and keep their marks. This supersedes the "9 of 123" figure carried until now, which was an undercount by half. **RADAR MOVED SIDES 2026-09-13, and the wholly-lost count went 8 → 9.** It sat in the arrows group from the day this row was written, and it has no arrows: showcase #107 is `radar` with `bars: true`, and its eight radial bars are eight `wedge` nodes. The split is now exactly 9 and 9. It was caught by re-deriving the figure instead of citing it, which is the only reason to prefer a recipe to a remembered number — `npm run mac:webkit` prints both groups on every run. **And the elements are not in that count**: a Harvey ball draws an empty ring at every fraction between 1% and 99% — 0% and 100% are ellipses and survive, so exactly the informative range is lost — and a table `[up]`/`[down]`/`[flat]` cell keeps its text and loses its arrow. A diagonal SEGMENT is fine: it falls back to a real line. The choice is refuse / warn / fall back to a picture, and the pane already has the machinery — the `stalled` path tells a user which charts came out blank. **Not urgent for the web**, which reports 1.10; this is desktop, Mac and every volume-licensed build. **AND THE BEHAVIOUR THIS ROW CHOSE IS NOW UNREACHABLE, 2026-09-13.** The owner raised the manifest floor to **1.10** that evening, so `canRotate()` is true on every host the manifest admits, `marksThisHostWillDrop` always returns empty, and `pictureForUndrawableMarks` — the warn-and-picture path chosen here on 2026-08-31 — can no longer fire. That is not this decision being reversed on its merits: it was the right call for a product that admitted 1.4 hosts, it shipped, and the floor moved out from under it. Recorded rather than struck, because the reasoning is what a future floor change would need. The code and its tests stay (`canRotate` is still gated, and its tests drive `supports()` directly rather than the shipped floor) precisely so this is recoverable if the floor ever drops again. **Do not read "the picture fallback is gone" off the floor change**: the DENSITY rescue in `wantsAutoPicture` and the user's own "insert as image" choice are untouched — only the undrawable-marks reason for making a picture went away. |
